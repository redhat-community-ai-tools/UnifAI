...
export async function checkResourceUsage(resourceId: string): Promise<ResourceUsageResponse> {
  const { data } = await axios.get<ResourceUsageResponse>(
    `/resources/resource.check-usage?resourceId=${encodeURIComponent(resourceId)}`,
  );
  return data;
}

export async function forceDeleteResource(payload: {
  resourceId: string;
  ...
  await axios.post("/resources/resource.force-delete", {
    ...
    mode: payload.mode,
    ...(payload.replacementId ? { replacementId: payload.replacementId } : {}),
  });
}
