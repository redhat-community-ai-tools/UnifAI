...
export const ElementGrid: React.FC<ElementGridProps> = ({
  ...
  const handleSelectionChange = (rid: string, checked: boolean) => {
    if (!onRowSelectionChange) return;
    const next = { ...rowSelection };
    if (checked) next[rid] = true;
      ...
      {elements.map((element, index) => {
        ...
        return (
          <RegularElementCard
            ...
            onValidationClick={() => handleValidationClick(element.rid)}
            isSelected={rowSelection[element.rid] === true}
            onSelectionChange={
              onRowSelectionChange
                ? (checked) => handleSelectionChange(element.rid, checked)
                : undefined
            }
          />
        );
