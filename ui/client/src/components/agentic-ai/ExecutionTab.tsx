...
export default function ExecutionTab({ runId, onSessionChange }: ExecutionTabProps): React.ReactElement {
  ...
  const {
    ...
    onSuccess: (deletedIds) => {
      ...
      hub.setChatSessions((prev) => prev.filter((s) => !idSet.has(s.id)));
      hub.setSelectedSession((prev) => {
        if (prev && idSet.has(prev.id)) {
          hub.setCurrentSessionMessages([]);
          return null;
        }
        return prev;
      });
  ...
  useEffect(() => {
    pruneToIds(new Set(hub.chatSessions.map((s) => s.id)));
  }, [hub.chatSessions, pruneToIds]);
  ...
  const allChatSessionsSelected = useMemo(
    () => hub.chatSessions.length > 0 && hub.chatSessions.every((s) => selection[s.id] === true),
    [hub.chatSessions, selection],
  );

  const selectAllChatSessions = useCallback(() => {
    setSelection((prev) => {
      const next = { ...prev };
      hub.chatSessions.forEach((s) => {
        next[s.id] = true;
      });
      return next;
    });
  }, [hub.chatSessions, setSelection]);
              ...
              <div className="flex justify-between items-center min-w-0 w-full max-w-full">
                ...
                <div className="flex items-center gap-1 flex-shrink-0 max-w-fit">
                  <SelectionModeControls
                    ...
                    bulkDeleteDisabled={bulkDeleteLoading || hub.isDeleting}
                    itemNameForDelete={selectedCount === 1 ? "chat session" : "chat sessions"}
                    totalSelectable={hub.chatSessions.length}
