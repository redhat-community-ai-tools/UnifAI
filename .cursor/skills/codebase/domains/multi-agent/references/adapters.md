...
| `MongoAdminConfigReader` reads the backend service's `config.admin_config` collection directly | `adapters/outbound/mongo/admin_config_reader.py`, implements `AdminConfigReaderPort` | Deliberate, documented exception to per-service Mongo ownership: MAS only needs a read-only `is_admin(username)` check against the same admin allowlist the backend's `AdminConfigService` owns writes to (`backend/admin_config/`). Avoids a network round-trip per request; MAS never writes to this collection. This is NOT the "cross-service repository access" anti-pattern (Service A using Service B's repository class) — it's a narrow, intentional read-only cross-database contract. See `backend/SKILL.md` Established Patterns for the mirrored entry. |
