"""
End-to-End Stress Test (UI-aligned: submit + status poll)

Copy of test_session_stress.py that runs sessions the same way the playground UI
does for long workflows:
...
Unlike the original file, this does NOT use user.session.execute (blocking or
streaming). Success means session status COMPLETED, not "HTTP stream finished".
...
class StressTestConfig:
    ...
    # Kept for CLI compat with original stress file; ignored here (always submit+poll).
    use_streaming: bool = False
    
    # Load Configuration
    num_sessions: int = 20  # Total sessions to create
    concurrent_create: int = 5  # Concurrent session creations
    concurrent_execute: int = 10  # Max in-flight submit+poll at peak
    ...
    # ramp_interval <= 0 disables ramp (submit up to concurrent_execute immediately).
    ramp_start: int = 1
    ramp_step: int = 1
    ...
    execution_timeout: float = 1800.0  # Max wait for one session to reach terminal status
    total_timeout: float = 3600.0  # Total test timeout
    poll_interval: float = 5.0  # Seconds between session.status.get polls
...
class StressTestRunner:
    ...
    def execute_session_with_metrics(
        ...
        """
        Submit a session (UI path) and poll until COMPLETED.
        ...
        try:
            result = self.client.submit_and_wait(session_id, inputs)
    ...
    def _record_execution_future(
        ...
        """Collect one completed submit+poll future and print outcome."""
        try:
            ...
            if success and result:
                ...
                print(
                    ...
                    f"{session_id[:8]}... {status} in {duration:.2f}s "
                    f"({polls} polls)",
                    flush=True,
                )
            else:
    ...
    def run_concurrent_execution(self, session_ids: List[str], inputs: Dict) -> List[Dict]:
        """
        Submit+poll sessions with optional concurrency ramp-up.
        ...
        if ramp_enabled:
            ...
            print(
                f"\n⚡ Submitting {total} sessions with RAMP-UP: "
            ...
            print(
                f"\n⚡ Submitting {total} sessions with concurrency="
                f"{max_concurrent} (no ramp; poll every {self.config.poll_interval}s, "
                f"timeout {self.config.execution_timeout:.0f}s)",
                flush=True,
            )
...
class TestSessionStressSubmit:
    """E2E stress tests using submit + status poll (UI-aligned)."""
    ...
    def test_concurrent_session_creation_and_execution(
        ...
        """
        Concurrent create + submit/poll until COMPLETED.
        """
        blueprint_id = None
        ...
        try:
            print(f"\n{'=' * 80}")
            print("🧪 STARTING E2E SESSION STRESS TEST (submit + poll)")
            print(f"{'=' * 80}")
            print(f"Configuration:")
            print(f"  • Total Sessions: {stress_config.num_sessions}")
            print(f"  • Concurrent Creates: {stress_config.concurrent_create}")
            print(f"  • Max concurrent submits: {stress_config.concurrent_execute}")
            ...
            print(f"  • Mode: submit + status poll")
            ...
            # PHASE 3: Submit + poll to COMPLETED
            print(f"\n{'=' * 80}")
            print("📋 PHASE 3: Submit + Poll Until COMPLETED")
            print(f"{'=' * 80}")
            ...
            print("\n" + "=" * 80)
            print("✅ STRESS TEST PASSED (submit + poll)")
            print("=" * 80 + "\n")
    ...
    def test_rapid_sequential_sessions(
        ...
        try:
            ...
            for i in range(num_rapid_sessions):
                ...
                # Submit + poll (UI path)
                result = api_client.submit_and_wait(session_id, test_inputs)
                ...
                print(
                    ...
                    f"({result.get('status')}, {result.get('polls')} polls)"
                )
...
class TestCustomBlueprintStressSubmit:
    """Stress tests using your custom blueprint (submit + poll)."""
