...
Covered surface:
  - update_draft()  — legacy (no version_repo), versioned, OCC failure, snapshot skip
...
# ── update_draft — legacy path (no version_repo) ──────────────────────────────


@pytest.mark.unit
class TestUpdateDraftLegacyPath:
    """When version_repo is None, update_draft falls back to repo.update()."""

    def test_calls_repo_update_and_returns_true(self):
        repo = MagicMock()
        ...
        # OCC method must NOT be called on the legacy path
        repo.update_with_version.assert_not_called()

    def test_raises_blueprint_not_found_when_load_raises_key_error(self):
        repo = MagicMock()
        repo.load.side_effect = KeyError("bp-1")

        svc = _make_service(repo=repo, version_repo=None)
        with pytest.raises(BlueprintNotFoundError) as exc_info:
            svc.update_draft(blueprint_id="bp-missing", draft_dict={"name": "X", "nodes": []})

        assert exc_info.value.blueprint_id == "bp-missing"
...
class TestUpdateDraftVersionedPath:
    ...
    def test_snapshot_failure_is_logged_but_not_re_raised(self):
        """A DuplicateKeyError on insert_snapshot is swallowed — OCC write still proceeds."""
        svc, repo, version_repo, _ = self._setup(current_version=2)
        version_repo.insert_snapshot.side_effect = Exception("DuplicateKeyError simulated")
