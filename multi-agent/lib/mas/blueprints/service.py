...
class BlueprintService:
    ...
    def _snapshot_version(
        ...
        Silently ignores ``DuplicateKeyError`` — this makes the operation
        idempotent so retried requests cannot create duplicate snapshots.
        """
        try:
            ...
            # Swallow duplicate-key errors for idempotency; re-raise anything else.
            exc_str = str(exc).lower()
            if "duplicate" in exc_str or "11000" in exc_str:
                return
            raise
